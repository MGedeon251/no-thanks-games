<template>
  <div class="relative min-h-screen">
    ...
    <Transition name="screen-fade" mode="out-in">
      ...
      <!-- ── Choix du mode ── -->
      <ModeSelect v-if="appPhase === 'mode-select'" key="mode-select"
        @solo="appPhase = 'solo-setup'" @network="handleNetworkMode" />

      <!-- ══════════ MODE SOLO ══════════ -->
      <SetupScreen v-else-if="appPhase === 'solo-setup'" key="solo-setup"
        :has-save="hasSave" @start="handleSoloStart" @resume="handleSoloResume" />

      <GameBoard v-else-if="appPhase === 'solo-playing' && soloCurrentCard !== null" key="solo-playing"
        :players="soloPlayers" :deck="soloDeck" :current-card="soloCurrentCard"
        :tokens-on-card="soloTokensOnCard" :current-player-index="soloCurrentPlayerIndex"
        :current-player="soloCurrentPlayer" :last-action="soloLastAction"
        :card-anim-key="soloCardAnimKey" :no-token-shake="soloNoTokenShake"
        :live-scores="soloLiveScores"
        :timer-enabled="timerEnabled" :remaining="timerRemaining" :progress="timerProgress" :urgency="timerUrgency"
        @take="handleSoloTake" @refuse="handleSoloRefuse" @quit="showQuitDialog = true" />

      <ScoreScreen v-else-if="appPhase === 'solo-finished'" key="solo-finished"
        :ranking="soloRanking" @restart="handleSoloRestart" @menu="appPhase = 'mode-select'" />

      <!-- ══════════ MODE RÉSEAU ══════════ -->
      <NetworkLobby v-else-if="appPhase === 'network-lobby'" key="network-lobby"
        :connected="netConnected" :connecting="netConnecting"
        :socket-error="netSocketError ? String(netSocketError) : null"
        :in-room="!!netRoomCode" :room-code="netRoomCode"
        :is-host="netIsHost" :my-player-name="netMyPlayerName"
        :players="netGameState?.players ?? []"
        :notification="netNotification" :action-error="netActionError"
      ...
      <NetworkGameBoard v-else-if="appPhase === 'network-playing' && netGameState?.currentCard != null" key="network-playing"
        :my-player-name="netMyPlayerName" :room-code="netRoomCode"
        :is-my-turn="netIsMyTurn" :my-player="netMyPlayer"
        :players="netGameState.players"
        :current-card="netGameState.currentCard" :tokens-on-card="netGameState.tokensOnCard"
        ...
        @take="netPrendreCarte" @refuse="netRefuserCarte" @leave="handleNetLeave" />

      <ScoreScreen v-else-if="appPhase === 'network-finished'" key="network-finished"
        :ranking="netRanking" @restart="appPhase = 'network-lobby'" @menu="appPhase = 'mode-select'" />

      <!-- ══════════ MODE SPECTATEUR ══════════ -->
      <SpectatorView v-else-if="appPhase === 'spectator'" key="spectator"
        :room-code="netRoomCode"
        :phase="netGameState?.phase ?? 'lobby'"
        :players="netGameState?.players ?? []"
        :current-card="netGameState?.currentCard ?? null"
        :tokens-on-card="netGameState?.tokensOnCard ?? 0"
        :cards-remaining="netGameState?.cardsRemaining ?? 0"
        :current-player-name="netGameState?.currentPlayerName ?? ''"
        :last-action="netGameState?.lastAction ?? null"
        :card-anim-key="netCardAnimKey"
        :timer-enabled="netTimerEnabled"
        :timer-remaining="netTimerRemaining"
        :timer-progress="netTimerProgress"
        :timer-urgency="netTimerUrgency"
        :ranking="netRanking"
        @leave="handleNetLeave" />
    ...
    <Transition name="dialog-fade">
      <div v-if="showQuitDialog" class="fixed inset-0 flex items-center justify-center z-50 px-4"
        style="background:rgba(0,0,0,0.6);backdrop-filter:blur(4px);" @click.self="showQuitDialog = false">
...
// ── Solo ──────────────────────────────────────────────────────────────────────
const {
  phase: soloPhase, players: soloPlayers, deck: soloDeck,
  currentCard: soloCurrentCard, tokensOnCard: soloTokensOnCard,
  currentPlayerIndex: soloCurrentPlayerIndex, currentPlayer: soloCurrentPlayer,
  lastAction: soloLastAction, cardAnimKey: soloCardAnimKey, noTokenShake: soloNoTokenShake,
...
onMounted(() => { try { hasSave.value = !!localStorage.getItem('no-thanks-save') } catch {} })
...
function handleSoloResume() { const ok = loadFromStorage(); if (!ok) hasSave.value = false }
function handleSoloRestart() {
  if (!soloPlayers.value.length) { appPhase.value = 'mode-select'; return }
  initGame(soloPlayers.value.map(p => p.name), soloPlayers.value.map(p => p.isAI))
}
function handleSoloTake()   { stopTimer(); soloPrendreCarte() }
function handleSoloRefuse() { stopTimer(); soloRefuserCarte() }
function doQuit() {
  showQuitDialog.value = false; stopTimer(); resetGame()
  hasSave.value = true; appPhase.value = 'mode-select'
}

// ── Réseau ────────────────────────────────────────────────────────────────────
const {
  connect: netConnect, connected: netConnected, connecting: netConnecting, socketError: netSocketError,
  phase: netPhase, roomCode: netRoomCode, isHost: netIsHost, isSpectator: netIsSpectator,
...
watch(netGameState, state => {
  if (!state) return
  if (state.phase === 'playing' && appPhase.value === 'network-lobby' && !netIsSpectator.value)
    appPhase.value = 'network-playing'
  if (state.phase === 'playing' && appPhase.value === 'network-lobby' && netIsSpectator.value)
    appPhase.value = 'spectator'
})
...
function handleNetConnect(url) { netConnect(url) }

function handleSpectate(code) {
  netSpectateRoom(code)
  appPhase.value = 'spectator'
}
function handleNetLeave() { netLeaveRoom(); appPhase.value = 'mode-select' }

// ── Déco ─────────────────────────────────────────────────────────────────────
function floatingTokenStyle(i) {
  const p = [
    {left:'5%',top:'15%',d:'0s',dur:'7s'}, {left:'88%',top:'8%',d:'1.2s',dur:'9s'},
    {left:'15%',top:'75%',d:'0.5s',dur:'8s'}, {left:'92%',top:'60%',d:'2s',dur:'6s'},
    {left:'45%',top:'5%',d:'0.8s',dur:'10s'}, {left:'70%',top:'90%',d:'1.5s',dur:'7.5s'},
    {left:'30%',top:'92%',d:'3s',dur:'8.5s'}, {left:'58%',top:'50%',d:'2.5s',dur:'9.5s'},
  ][(i-1)%8]
